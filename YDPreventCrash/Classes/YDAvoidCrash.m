...
static void(^YDAvoidCrashBlock)(NSException *exception,NSString *defaultToDo,NSDictionary *info);
 ...
 * 设置信息回掉收集
 */
+ (void)setupBlock:(void(^)(NSException *exception,NSString *defaultToDo,NSDictionary *info))aBlock {
...
+ (void)noteErrorWithException:(NSException *)exception defaultToDo:(NSString *)defaultToDo upload:(BOOL)upload {
    ...
    if (YDAvoidCrashBlock) {
        YDAvoidCrashBlock(exception,defaultToDo,infoDic);
    }
